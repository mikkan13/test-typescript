...
function Contact() {
  ...
  const onSubmit: FormEventHandler<HTMLFormElement> = (e) => {
    ...
    if (insufficientNameLength)
      setNameErrorMessage("30文字以内で入力してください。");
    if (emptyEmail) setEmailErrorMessage("メールアドレスは必須です。");
    if (!regex.test(email))
      setEmailErrorMessage("emailの形式で入力してください。");
    if (emptyMessage) setMessageErrorMessage("本文は必須です。");
    if (insufficientMessageLength)
      setMessageErrorMessage("本文は500文字以内で入力してください。");

    const enableSubmit =
      !emptyName &&
      !insufficientNameLength &&
      !emptyEmail &&
      regex.test(email) &&
      !emptyMessage &&
  ...
  return (
    <div className="mx-auto mb-10 w-8/12 py-10">
      <h1 className="text-xl text-black-500 font-bold mb-10">
        問合わせフォーム
      </h1>
      <form onSubmit={onSubmit}>
        <div className="md:flex items-center mb-6">
          <div className="md:w-1/4">
            <label
              className="block mb-1 md:mb-0 pr-4 text-black"
              htmlFor="name"
            >
              お名前
            </label>
          ...
          <div className="md:w-3/4">
            ...
            />
            {nameErrorMessage && (
              <span className="text-red-600 text-sm">{nameErrorMessage}</span>
            )}
          </div>
        </div>
        <div className="md:flex items-center mb-6">
        ...
        <div className="flex items-center justify-center">
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded mx-2 rounded-lg">
            送信
          </button>
          <button
            className="font-bold bg-gray-200 hover:bg-gray-300 px-4 py-2 rounded mx-2 rounded-lg"
            onClick={inputClear}
            type="button"
          >
            クリア
